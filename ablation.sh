...
JUDGE_MODELS="${JUDGE_MODELS:-meta-llama/Llama-3.1-8B-Instruct google/gemma-7b-it}"
...
TARGET_LAYER_INDICES="${TARGET_LAYER_INDICES:-${TARGET_LAYER_INDEX:-}}"
...
TARGET_LAYER_INDICES="${TARGET_LAYER_INDICES:-$(read_export_value SR_TARGET_LAYER_INDICES 27 "${TEMPLATE_ENV_PATH}")}"
...
TARGET_LAYER_NAME="indices_$(safe_name "${TARGET_LAYER_INDICES}")"
...
export JUDGE_MODEL=${MAIN_JUDGE_MODEL}
export HALLUCINATION_JUDGE_MODEL=${MAIN_JUDGE_MODEL}
...
export SR_TARGET_LAYER_INDICES=${TARGET_LAYER_INDICES}
...
EOF_ENV
  ...
  if [[ "${resolved_judge}" != "${MAIN_JUDGE_MODEL}" || "${resolved_hall}" != "${MAIN_JUDGE_MODEL}" || "${resolved_eval_judge}" != "${judge_model}" ]]; then
    echo "[ablation] judge override sanity failed" >&2
    echo "  wanted_judge=${MAIN_JUDGE_MODEL}" >&2
    echo "  wanted_hallucination_judge=${MAIN_JUDGE_MODEL}" >&2
    echo "  wanted_eval_judge=${judge_model}" >&2
  ...
  cat > "${run_dir}/run_config.json" <<EOF_JSON
{
  ...
  "judge_model": "${MAIN_JUDGE_MODEL}",
  "hallucination_judge_model": "${MAIN_JUDGE_MODEL}",
...
EVAL_EVERY_STEPS=${EVAL_EVERY_STEPS_FOR_RUN}
EVAL_START_STEP=${EVAL_START_STEP_FOR_RUN}
SR_TARGET_LAYER_INDICES=${TARGET_LAYER_INDICES}
